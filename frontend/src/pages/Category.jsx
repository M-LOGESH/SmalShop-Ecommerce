...
const categories = [
    ...
    },
    {
        ...
        items: [
            ...
            { name: 'Repellents', img: '/img/categories/repellent.webp', slug: 'repellents' },
...
function Category() {
    ...
    return (
        <div className="mx-auto min-h-screen max-w-6xl px-4 py-6 sm:px-10">
            {categories.map((category, idx) => (
                <div key={idx} className="mb-10">
                    <h2 className="mb-4 text-md sm:text-lg font-bold">{category.title}</h2>
                    ...
                    <div className="grid grid-cols-4 gap-3 sm:grid-cols-5 sm:gap-6 md:grid-cols-6 lg:grid-cols-8">
                        {category.items.map((item, i) => (
                            ...
                            >
                                <div className="flex h-16 w-16 items-center justify-center rounded-xl bg-slate-200 p-1 shadow-md transition hover:shadow-lg sm:h-24 sm:w-24">
                                    <img
                                        src={item.img}
                                        alt={item.name}
