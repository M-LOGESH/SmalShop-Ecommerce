...
import { FaHeart, FaChevronRight, FaPlus, FaMinus } from "react-icons/fa";
...
function CategoryProducts({ categoryName, title }) {
  ...
  // --- Load wishlist & cart for current user ---
  useEffect(() => {
    if (user?.access) {
      loadWishlist();
      loadCart();
  ...
  const updateCartQuantity = async (cartId, newQty) => {
    try {
      if (newQty <= 0) {
        await fetchWithAuth(`http://127.0.0.1:8000/api/cart/${cartId}/`, { method: "DELETE" });
          ...
          {products.map((p) => {
            ...
            return (
              ...
              >
                ...
                {cartItem ? (
                  <div className="mt-2 flex items-center justify-between rounded bg-violet-900 font-bold px-2 py-1 text-sm text-white">
                    <button
                      onClick={() =>
